// ClusterPIBT — copy of PIBT logic for cluster-based variant.
...
ClusterPIBT::ClusterPIBT(const Instance* _ins, DistTable& _D, std::mt19937* _MT,
                         uint _time_window)
    : ins(_ins),
      D(_D),
      MT(_MT),
...
ClusterPIBT::~ClusterPIBT()
{
  for (auto a : A) delete a;
}

void ClusterPIBT::record_interaction(uint a, uint b, uint time)
{
  if (a != b) {
...
void ClusterPIBT::compute_clusters()
{
  clusters.clear();
...
const std::vector<std::vector<uint>>& ClusterPIBT::get_clusters(uint time) const
{
  const uint window_idx = time / time_window;
...
uint ClusterPIBT::get_edge_cost(const Config& C1, const Config& C2) const
{
  uint cost = 0;
  for (uint i = 0; i < N; ++i) {
...
bool ClusterPIBT::get_new_config(HNode* H, LNode* L, Config& C_new)
{
...
RolloutResult ClusterPIBT::rollout(HNode* H)
{
...
bool ClusterPIBT::funcPIBT(Agent* ai)
{
  const auto i = ai->id;
...
Agent* ClusterPIBT::swap_possible_and_required(Agent* ai)
{
  const auto i = ai->id;
...
bool ClusterPIBT::is_swap_required(const uint pusher, const uint puller,
...
bool ClusterPIBT::is_swap_possible(Vertex* v_pusher_origin, Vertex* v_puller_origin)
{
  auto v_pusher = v_pusher_origin;
  auto v_puller = v_puller_origin;
  Vertex* tmp = nullptr;
